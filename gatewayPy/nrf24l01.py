...
class RH_NRF24:
    ...
    def initForMirf(self):
        self.nrf24SPI.speed_hz = 1000000
        self.setRXAddress(1,'serv')
        self.setTXAddress('cln1')
        self.setChannel(100)
        self.setPipeLength(0,5)
        self.setPipeLength(1,5)
        self.setModeRx()   #self.writeReg(CONFIG,NRF24_EN_CRC|NRF24_PWR_UP|NRF24_PRIM_RX) #    
        self.writeReg(STATUS, RH_NRF24_TX_DS | RH_NRF24_MAX_RT) #RH_NRF24_RX_DR |
        self.writeReg(EN_AA, 63) # disable ACK
        self.flushTx()
        self.flushRx()
        self.setRF(self.DataRate2Mbps,self.TransmitPower0dBm)        
        return True
    
    def initForMIrfTx(self):
        self.nrf24SPI.speed_hz = 1000000
        self.setRXAddress(1,'cln1') #rx address
        self.setRXAddress(0,'serv') #rx address
        self.setTXAddress('serv') #TX address
        self.setChannel(1)
        self.setPipeLength(0,5)
        self.setPipeLength(1,5)
        self.setModeTx() 
        self.writeReg(STATUS, RH_NRF24_TX_DS | RH_NRF24_MAX_RT)
        self.flushTx()
        self.flushRx()
        self.setRF(self.DataRate2Mbps,self.TransmitPower0dBm)
        return True
    ...
    def sendMIRF(self,data):
        _bytes = [COMMAND_W_TX_PAYLOAD_NOACK] 
        _bytes.extend([255]) #master node address
        _bytes.append(randint(10,100))
        _bytes.append(127)
        _bytes.append(136)
        _bytes.append(142)
        self.setModeTx()
    ...
    def rcvMRFI(self):
        #print(self._bufLen)
        message = []
        for x in range(0,self._bufLen+1): #.decode("utf-8")
             if x > 0:                  
                  #print((self._buf[0][x]))
    ...
    def available(self,readRegister):
        if not self._rxBufValid:
            ...
            self.burstRead(readRegister,self._buf,len)
    ...
    def setRXAddress(self,pipeNr,address):
        bytes = [WRITE_REG|(RX_ADDR_P0+pipeNr)]
        tmp = []
        for x in address:
            tmp.append(ord(x))
        bytes.extend(tmp)
    ...
    def printRegisterMap(self):
        ...
        for x in range(0,17):
            ...
            print(int(SendObj.readReg(x),16))
